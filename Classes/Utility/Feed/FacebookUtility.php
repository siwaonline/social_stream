...
class FacebookUtility extends \Socialstream\SocialStream\Utility\Feed\FeedUtility
{
    ...
    {
        $url = "https://graph.facebook.com/".$channel->getObjectId(
            )."/?fields=id,name,about,link,picture,cover&access_token=".$channel->getToken();
        if ($this->get_http_response_code($url) == 200) {
            ...
            if ($elem->about) {
                $channel->setAbout($elem->about);
            }
            //if($elem->description)$channel->setDescription($elem->description);
            $channel->setLink($elem->link);

            if ($isProcessing == 0) {
                $picStream = json_decode(
                    file_get_contents(
                        "https://graph.facebook.com/".$channel->getObjectId(
                        )."/picture?redirect=0&width=900&access_token=".$channel->getToken()
                    )
                );
    ...
    public function getFeed(\Socialstream\SocialStream\Domain\Model\Channel $channel, $limit = 100)
    {
        ...
        if ($channel->getPosttype() == "1") {
            $url = "https://graph.facebook.com/".$channel->getObjectId(
                )."/posts?fields=id,created_time,link,permalink_url,place,type,message,full_picture,object_id,picture,name,caption,description,story,source,from&access_token=".$channel->getToken(
                )."&limit=".$limit;
        } else {
            $url = "https://graph.facebook.com/".$channel->getObjectId(
                )."/feed?fields=id,created_time,link,permalink_url,place,type,message,full_picture,object_id,picture,name,caption,description,story,source,from&access_token=".$channel->getToken(
                )."&limit=".$limit;
        }
            ...
            if ($entry->name || $entry->message) {
                ...
                if ($entry->message) {
                    ...
                    if ($entry->description) {
                        ...
                        if ($entry->caption) {
                            $news->setAlternativeTitle($entry->caption);
                        }
                    } else {
                        if ($entry->caption) {
                            $news->setTeaser($entry->caption);
                        }
                    }
                } else {
                    if ($entry->description) {
                        ...
                        if ($entry->caption) {
                            $news->setBodytext($entry->caption);
                        }
                    }
                }
                if ($entry->story) {
                    $news->setDescription($entry->story);
                }
                ...
                $singlePost = json_decode(
                    file_get_contents(
                        "https://graph.facebook.com/".$entry->id."/?fields=full_picture,source&access_token=".$channel->getToken(
                        )
                    )
                );
