...
class FeedUtility extends \Socialstream\SocialStream\Utility\BaseUtility
{
    ...
    protected function processChannelMedia(\Socialstream\SocialStream\Domain\Model\Channel $channel, $imageUrl)
    {
        $folder = $this->getSubFolder(
            $this->getSubFolder($this->getMainFolder(), $channel->getType()),
            $channel->getObjectId()
        );
    ...
    protected function processNewsMedia(\Socialstream\SocialStream\Domain\Model\News $news, $imageUrl)
    {
        $folder = $this->getSubFolder(
            $this->getSubFolder(
                $this->getSubFolder($this->getMainFolder(), $news->getChannel()->getType()),
                $news->getChannel()->getObjectId()
            ),
            "news"
        );
    ...
    protected function processMedia($model, $table, $field, $folder, $imageUrl)
    {
        ...
        if ((!$folder->hasFile($imageName) && $imageName) || ($storage->getFileInFolder($imageName, $folder)->getSize(
                ) <= 0 && $folder->hasFile($imageName) && $imageName)) {
            $this->grab_image($imageUrl, $this->settings["tmp"].$imageName);
            if (filesize($this->settings["tmp"].$imageName) > 0) {
                $movedNewFile = $storage->addFile(
        ...
        if ($image) {
            ...
            $tce = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(
    ...
    {
        if (strpos($url, 'safe_image.php') !== false
            && strpos($url, '&url=') !== false
            && strpos($url, '&url=fbstaging') === false) {

            $urlpath = preg_replace_callback(
                '/\\\\u([0-9a-fA-F]{4})/',
                function ($match) {
     ...
     */
    public function sendTokenInfoMail(
        \Socialstream\SocialStream\Domain\Model\Channel $channel,
        $sysmail,
        $sendermail = ""
    ) {
        ...
        $subject = "Social Stream - ".\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate(
                'labels.subject_token',
                'social_stream'
            );
