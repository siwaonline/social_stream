...
class FacebookUtility extends \Socialstream\SocialStream\Utility\Token\TokenUtility
{
    ...
    {
        $url = "https://www.facebook.com/v2.4/dialog/oauth?client_id=" . $this->settings["fbappid"] . "&state=" .
            $this->settings["fbappsecret"] . "&response_type=token&scope=user_posts&sdk=php-sdk-5.0.0&redirect_uri=";
        ...
        if (!$params["access_token"]) {
            ...
            $token = file_get_contents(
                "https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=" .
                    $this->settings["fbappid"] .
                    "&client_secret=" . $this->settings["fbappsecret"] . "&fb_exchange_token=" . $params["access_token"]
            );
