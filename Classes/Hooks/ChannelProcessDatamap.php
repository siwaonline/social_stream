    ...
    function processDatamap_postProcessFieldArray($status, $table, $id, &$fieldArray, &$reference)
    {
        ...
        if ($table == 'tx_socialstream_domain_model_channel') {
            ...
            if (array_key_exists('type', $fieldArray)) {
                $channel->setType($fieldArray["type"]);
            }
            if (array_key_exists('object_id', $fieldArray)) {
                $channel->setObjectId($fieldArray["object_id"]);
            }
            if (array_key_exists('token', $fieldArray)) {
                $channel->setToken($fieldArray["token"]);
            }
            if ($channel) {
